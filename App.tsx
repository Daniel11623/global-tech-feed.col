...
const App: React.FC = () => {
  ...
  const loadAndTranslateNews = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const fetchedNews = await fetchLatestTechNews();
      ...
      const processedArticles = await Promise.all(
        fetchedNews.map(async (articleData): Promise<Article> => {
          ...
          if (articleWithDefaults.lang === 'en') {
            try {
              const translatedContent = await translateArticleToSpanish(articleWithDefaults);
              ...
              return articleWithDefaults; // Return original if translation fails
            }
          }
          return articleWithDefaults; // Return Spanish articles as is
        })
      );
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className="bg-red-900/50 border border-red-500 text-red-300 p-4 mb-6 rounded-lg text-center" role="alert">
            <p className="font-bold">Error</p>
            <p className="mb-4">{error}</p>
            <button
                onClick={handleRetry}
                className="inline-block border-2 border-red-300 text-red-300 font-bold py-2 px-5 rounded-lg hover:bg-red-300 hover:text-background transition-colors duration-300"
            >
                Reintentar
            </button>
        </div>
